...
class LensSystem:
    ...
    #     ax.set_xlim([min(z_list) - 0.3*length-100, 0.3*length+20])
    ...
    # 光路図をプロットする
    # n_rays: レイの本数
    def optical_path_diagram(self, n_rays=50000):
        # レンズ系の表示
        fig, ax = plt.subplots()
        #ax = self.plot()
        ...
        # 入射レイ
        # ベクトル化された実装
        y = torch.arange(n_rays, device = device).unsqueeze(dim = 0) # 全てのx, y座標を生成
        y = (y - n_rays / 2) / n_rays
        z = torch.sqrt(torch.tensor(1, device = device)).expand_as(y)
        ray_roi = torch.ones_like(y).unsqueeze(dim = 2)
        ray_intensity = torch.ones_like(y).unsqueeze(dim = 2)

        ray_direction = torch.stack([torch.tensor(0, device = device).expand_as(y), y, z], dim = 0)
        ray_direction = ray_direction / torch.norm(ray_direction, dim=0, keepdim=True)
        ray_direction = torch.permute(ray_direction, (1,2,0))
        ray_origin = torch.tensor([0, pixel_size * IMAGESIZE /2, image_z], device = device).expand_as(ray_direction)

        ray_in = Ray(ray_origin, ray_direction, ray_roi, ray_intensity)
        ...
        for n in range(n_rays):
            ...
            line_y = []
            for ray in rays:
                line_x.append(ray[:,n,2].item())
                line_y.append(ray[:,n,1].item())
            #print(line_y)
            # プロット
            ax.plot(line_x, line_y, c='lime', linewidth = 0.001)
 
        # # zとyの抽出
        # line_x = list(map(lambda x: x.origin[2].cpu().numpy(), rays))
        # line_y = list(map(lambda x: x.origin[1].cpu().numpy(), rays))
    ...
    #     # レイトレース計算
    #     result_rays = self.raytrace_from_object(ray_in, mask = mask)
    ...
    #     ini_mask = (Image.open(f"0_initial_mask_128.png").convert("L"))
