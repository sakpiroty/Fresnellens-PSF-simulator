...
def save_tensorimage(tensor, filepath):
    # テンソルが2次元であることを確認
    if tensor.ndim != 2:
...
class Ray:
    ...
    # origin: レイの始点
    # direction: レイの方向ベクトル
    def __init__(self, origin: torch.tensor, direction: torch.tensor, roi: torch.tensor):
        self.origin = origin
...
class LensSurface:
    ...
    def intersect_cone(self, ray: Ray, index_of_cone: int) -> Optional[Tuple[torch.tensor, torch.tensor]]:
        ...
        # bunshi = mask1.sum().item()
        # bunbo = mask1.numel()
        # print(f'TEST RATIO: {bunshi/bunbo}')
        # bunshi = mask2.sum().item()
        # bunbo = mask2.numel()
        # print(f'TEST RATIO: {bunshi/bunbo}')
...
class LensSystem:
    ...
    # ray_in: 入射レイ
    def raytrace_from_object(self, ray_in: Ray) -> torch.tensor:
        ray = ray_in
        rays = [ray]
        lens = self.lenses[0]
        N_loop = 20
    ...
    def simulatePSF(self, n_rays = NRAYS):
        s_time = time.time()
        PSFSIZE = 500
        PSFNUM = 5
        #PSFX= 1 ## 1 ~ 5, CENTER: 3
        ...
        psftensor = torch.zeros((PSFSIZE, PSFSIZE), dtype=torch.long, device = device)
        ...
        ray_in = Ray(rayin_origin, rayin_direction, rayin_roi)

        # レイトレース計算
        result_rays = self.raytrace_from_object(ray_in)
        result_xys = result_rays[-1].origin/pixel_size/MAGNIFY
        # result_xys[:,:,0] = result_xys[:,:,0] + (PSFX - PSFCENTER)/2*IMAGESIZE/2 + PSFSIZE/2
        # result_xys[:,:,1] = result_xys[:,:,1] + (PSFY - PSFCENTER)/2*IMAGESIZE/2 + PSFSIZE/2
        result_xys[:,:,0] = result_xys[:,:,0] + PSFSIZE/2
        result_xys[:,:,1] = result_xys[:,:,1] + PSFSIZE/2
        # nan mask (迷光など)
        mask = ~torch.isnan(result_xys[:,:,0])
        # 有効なx座標とy座標を取得して四捨五入
        x_coords = (result_xys[:,:,0][mask] + 0.5).to(torch.long)
        y_coords = (result_xys[:,:,1][mask] + 0.5).to(torch.long)
        #print(x_coords)
        #print(y_coords)
        ...
        #print(x_coords)
        #print(y_coords)
        psftensor.index_put_((x_coords, y_coords), torch.ones_like(x_coords), accumulate=True)
        #print(psftensor)
        save_tensorimage(psftensor, "womaskPSF.png")
        ...
        for i, psf in enumerate(psfpatches):
            save_tensorimage(psf, filepath = f"./psfs/womask_{i+1}.png")

        psfimage = psftensor.cpu().numpy()
        psfimage = psfimage / np.sum(psfimage)
        
        e_time = time.time()
        print(f"ELAPSED TIME: {e_time - s_time}")

        # 結果をプロット
        plt.figure()
        plt.title("PSF Image")
        plt.imshow(psfimage, cmap='gray', vmin = 0)
        #plt.imshow(psfimage)
        plt.colorbar()
        plt.savefig("simulatedPSFGPU5*5.png")
        #plt.show()
