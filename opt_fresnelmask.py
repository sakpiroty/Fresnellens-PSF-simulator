...
class Ray:
    ...
    def copy(self) -> torch.tensor:
        ray = self.origin + t * self.direction
        return p
...
class LensSystem:
    ...
    def simulatePSF(self, ray: Ray, mask: torch.tensor, psfloc) -> torch.tensor:
        ...
        # tmp_ray = tmp_ray.intensity[:,:,0].clone()
        ...
        transmission = (tmp_ray.intensity[:,:,0].sum())/(tmp_intensity.sum()) # 光線の減衰比率
        print(f"transmission rate: {transmission}")

        lens = self.lenses[0]
        # 結像位置まで計算
        p, n, t = lens.intersect_plane(tmp_ray, object_z, 0, 500)
        # ray.origin = p
    ...
    def optim_mask(self):
        ...
        for i_step in range(NSTEPS):
            ...
            psftensor, transmission = self.simulatePSF(ray_behind_Fresnel[psfloc], mask = x, psfloc = psfloc)
            # reporter.report()
            ...
            loss = lossVarMTF + 3 *lossTRANS + 0.0001*lossShadow
